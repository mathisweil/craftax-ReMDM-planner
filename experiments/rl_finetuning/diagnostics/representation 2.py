...
# ---------------------------------------------------------------------------
# Activation norm statistics (pure JAX)
# ---------------------------------------------------------------------------


def make_activation_norm_fn(
    model_apply: Callable,
    schedule_fn: ScheduleFn,
    num_actions: int,
) -> Callable:
    """Build a JIT-compiled activation norm statistics function.
    ...
    Returns:
        JIT-compiled fn(params, obs, acts, rng) -> (mean, std, p50, p90).
    """
    ...
    def activation_norms(
        ...
        Returns:
            Tuple of (mean, std, p50, p90) as JAX scalars.
        """
        B = obs.shape[0]
        rng, t_rng, mask_rng = jax.random.split(rng, 3)
        ...
        mean = jnp.mean(norms)
        std = jnp.std(norms)
        p50 = jnp.percentile(norms, 50)
        p90 = jnp.percentile(norms, 90)
